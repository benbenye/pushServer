    ...
    $.ajax({
        ...
        success: function (returnData) {
            if (returnData != null && returnData.member_id > 0) {
                ...
                $("#login_user_name").html(returnData.nickname);
        ...
        success: function (i) {
            if (i.status == 1) {
                ...
                if ((Number(num) - 1) <= 0) {
                    ...
                    $('div.header_cart a.cart_a span.num').html(Number(num) - 1);
                }
...
//加入购物车
function add_cart_fly(cart_obj, pid_img_url, pid, count, sku_code, is_show_alert) {
...
//加入购物车 M版本
function add_cart_fly_m(pid, count, sku_code, callback) {
    //执行js
    $.ajax({
        type: 'post',
        dataType: 'json',
